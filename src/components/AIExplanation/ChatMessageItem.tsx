...
const ToolEventList: React.FC<{ events: AgentToolEvent[] }> = ({ events }) => (
  <div className="p-2 space-y-1.5 bg-[#EFEFEC] text-[11px] font-mono border-t border-black/10 max-h-48 overflow-y-auto">
    {events.map((evt, idx) => (
      <div key={idx} className="p-1.5 rounded bg-black/[0.05] border border-black/10">
        <div className="flex items-center space-x-1.5 text-zinc-800 font-semibold">
          {toolIcon(evt.name)}
          <span>{evt.name}</span>
          <span className="text-zinc-700 font-normal text-[10px] truncate">
            {evt.args ? JSON.stringify(evt.args) : ''}
          </span>
        </div>
        {evt.output && (
          <pre className="mt-1 p-1.5 rounded bg-zinc-100 text-zinc-800 text-[10px] whitespace-pre-wrap max-h-32 overflow-x-auto">
            {evt.output}
          </pre>
        )}
      </div>
    ...
    <div className={`flex items-start space-x-2.5 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
      ...
      >
        {isAssistant && msg.reasoning && (
          <div className="mb-2.5 rounded-lg border border-zinc-400 bg-[#FFFFFF] overflow-hidden text-xs">
            <details className="group">
              <summary className="flex items-center justify-between px-3 py-1.5 bg-zinc-100 cursor-pointer select-none text-zinc-800 hover:text-zinc-800 hover:bg-zinc-200 transition">
                ...
                <span className="text-[10px] text-zinc-700 group-open:hidden">点击展开</span>
              </summary>
      ...
      {!isAssistant && (
        <div className="w-6 h-6 rounded-full bg-zinc-100 border border-black/15 flex items-center justify-center shrink-0 mt-0.5">

...
export const Accordion: React.FC<AccordionProps> = ({
  ...
  <div className={`rounded-xl overflow-hidden ${tone.shell}`}>
    <div
      onClick={onToggle}
      className={`flex items-center justify-between px-3.5 py-2.5 cursor-pointer select-none transition border-b border-black/10 ${tone.header}`}
    >
    ...
    {isOpen && <div className={`border-t border-black/10 ${tone.body}`}>{children}</div>}
  </div>
);

...
明确说明修改对外部依赖、调用方或工程配置的实际影响。`,
  ...
  pseudocodePrompt: `你是一位代码伪代码化与语义精简专家。请将用户给出的 Diff 改动块中的【删除行 (-)】与【新增行 (+)】逐句重写为地道、精炼的中文自然语言伪代码。

【严格格式要求】：
1. 必须严格以 Diff 格式输出，每一行删除的伪代码必须以 '-' 开头，每一行新增的伪代码必须以 '+' 开头；
2. 保持与原改动行数量大体一致的 - 和 + 行；
3. 用通俗直白的大白话概括这一行代码做的事情（例如：- // 定义静态方法 LogConversion；+ // 废弃该方法，统一接入统一日志中心）；
4. 严禁输出任何 Markdown 标题、说明文字、反引号或开场白，直接输出转换后的 Diff 行。`,

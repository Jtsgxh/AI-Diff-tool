...
【用户配置的业务分析与讲解要求】
${learnPrompt}`;
  const routeBoundaryRules = isDrilldown
    ? `【当前递归粒度】本轮只展开所选路线步骤的内部执行，不重新划分仓库顶层功能线。子路线仍须从进入该步骤追到它对上层可观察的产出。`
    : isExpansion
      ? `【补充功能线边界】本轮只判断用户点名的缺失功能，不重新枚举整个仓库。仍以“触发入口 + 业务目标 + 结果或状态生命周期”识别独立路线；不得因为它与已有路线共用服务、AI 引擎、数据库、队列或工具层，就把它并入已有路线。`
    : `【顶层功能线边界】
- 先从用户操作、外部请求、命令、定时任务或消息入口枚举候选功能线，再逐条追到各自可观察的结果。
- 一条功能线由“触发入口 + 业务目标 + 结果或状态生命周期”共同确定。业务目标不同，或最终产物及其状态生命周期不同，就必须拆成不同的 businessRoute；不能因为中间共用同一个服务、AI 引擎、数据库、队列或工具层而合并。

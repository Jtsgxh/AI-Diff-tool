...
- 后台以仓库为单位分别维护审查、学习两条持续对话：直接解释、块释义、伪代码、关联分析和追问共用审查历史；仓库学习、补图和钻取共用独立学习历史。每条对话内排队执行，两条对话及不同仓库可并行。
- 对话保存完整模型消息、推理和工具结果，存放在用户目录的 `.ai-diff-tool/conversations`，重启后恢复；可用 `AI_DIFF_CONVERSATION_DIR` 指定目录。请求头与 API Key 不保存。
- 顶栏及手机“更多操作”在审查页提供“清除仓库对话”，学习页提供“清除学习对话”：取消当前对话线正在执行和排队的分析，并清除这条后台历史；另一条对话、已生成报告和本地结果缓存仍保留。失败或取消的分析不写入历史。
- 历史达到当前上下文预算时提示手动清除或调大设置，不自动压缩或截断旧历史。固定公共提示词，后续任务在历史末尾追加；仅关联探查发送工具定义，直接解释和综合输出不发送工具声明。直接解释仅收到思考且结束原因为 `stop` 时保留推理并补一次正文，再失败则停止；实际缓存收益取决于模型服务端。
- 支持复制完整报告与追问记录。

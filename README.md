...
业务路线生成采用同一条流中的两个阶段：先以 JSON Output 生成并严格校验 `learn-graph` 机器数据，校验通过后立即绘图；再单独流式生成中文讲解，避免正文干扰机器数据格式。中文讲解只有返回服务端约定的完整性结束标记后才会被判定完成；即使兼容端点错误返回 `finish_reason=stop`，缺少结束标记时也会从中断处自动续写，内部标记不会显示在报告中。
